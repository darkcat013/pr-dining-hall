...
func StartRatingLogging() {
	...
	for {
		select {
		case c := <-ClientRatingChan:
			atomic.AddInt64(&CompletedOrders, 1)
			atomic.AddInt64(&summedRatings, int64(c))
			...
			AvgRating = avg
			utils.LogRep.Info("AVG RATING", zap.Float64("rating", AvgRating), zap.Int("currentOrders", CurrentOrders), zap.Int("currentMaxOrders", CurrentMaxOrders))
